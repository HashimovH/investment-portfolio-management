...
async def test_get_all_active_stocks(repository, db_session):
    ...
    for stock in test_stocks:
        await db_session.execute(
            insert(Stock).values(
                name=stock.name, price=stock.price, active=stock.active
            )
        )
...
async def test_get_one_stock(repository, db_session):
    ...
    await db_session.execute(
        insert(Stock).values(
            name=test_stock.name, price=test_stock.price, active=test_stock.active
        )
    )
    await db_session.commit()
...
async def test_create_new_transaction_and_decrease_balance(repository, db_session):
    ...
    await db_session.commit()
    result = await repository.create_new_transaction_and_decrease_balance(
        test_client.id,
        test_client.balance,
        test_stock.id,
        10,
