...
async def test_get_all_active_stocks(service, repository):
    ...
    service._repository.get_all_active_stocks.return_value = [Stock(id=1,name="test",price=1.0, active=True)]
    result = await service.get_all_stocks()
...
async def test_get_user_transaction(service, repository, user_repository):
    ...
    # Test with data
    transaction_1 = Transactions(id=1, client_id=1, stock_id=1, volume=3, price=30.0, purchase_price=10.0, created_at=datetime.datetime.now())
    transaction_2 = Transactions(id=2, client_id=1, stock_id=1, volume=2, price=30.0, purchase_price=15.0, created_at=datetime.datetime.now())
    repository.get_user_transactions.return_value = [
        ([transaction_1, "Test Stock 1", 20,]),
...
async def test_create_new_transaction(service, repository):
    ...
    repository.get_stock.return_value = Stock(id=1, name="test", price=20.5, active=True)
